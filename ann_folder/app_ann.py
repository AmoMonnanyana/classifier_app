...
from map.geofile import generate_map
...
@app_ann.route("/upload_map", methods=['POST', 'GET'])
def upload_map():
    if request.method == "POST":
        map_uploads = current_app.config['UPLOAD_DIRECTORY_2']
        shapefile = request.files['shapefile']
        shapeName = secure_filename(shapefile.filename)
        session['shapeName'] = shapeName

        if shapefile:
             shapefile.save(os.path.join(map_uploads, shapeName))
        return render_template("index-soil-prediction.html")
    else:
        return render_template("index-soil-prediction.html")
    ...
    try:
        if session['username']:    
            ...
            if session['input_status'] == 'add_more':
                ...
                input_set['prediction_plot'] = y_predicted_classes
                print(input_set)
...
def read_file(new_file):
    try:
        if session['username']:
            ...
            dataset['prediction_plot'] = y_predicted_classes
...
@app_ann.route("/map", methods=['POST', 'GET'])
def generate():
    if request.method == 'POST':
        datatype = session['method']
        fiona.os.environ['SHAPE_RESTORE_SHX'] = 'YES'
        if datatype == 'input':
            data = input_results.query.all()
        elif datatype == 'upload':
            data = file_data.query.all()
        
        shapefile = session['shapeName']
        shapefile_path = f'map_uploads/{shapefile}'
        print(f"shape name is {shapefile_path}")
        print("shapefile accessed")
        data_argument = [(value.lat, value.long, value.cd, value.cr, value.ni, value.pb, value.zn, value.cu, value.co, value.predicted_mCdeg, value.predicted_class, value.prediction_plot) for value in data]
        
        input_set = pd.DataFrame(data_argument, columns=["lat", "long", "cd", "cr", "ni", "pb", "zn", "cu", "co", "predicted_mCdeg", "predicted_class", "prediction_plot"])
        generate_map(shapefile_path, input_set)
